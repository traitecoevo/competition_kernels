...
% TODO: Given this word in the title, it somehow feels strange that we rarely use it in main text -- comes out as "competition function" and similar
...
% Wondering here if this is a good place to cite Dolphs "Adaptive
...
% GEORGES: I have the feeling that 'traits' are coming out of the blue here
% a little bit. Later on we explain the link between traits and resources
% but may be we need to explained it here already. This is difficult as we
% really need to introduce trait here are they are central in the next section
...
% MW: I'm struggling to see this as a third line on the same question. It's true that Grime makes predictions about strategy-mixtures in communities, but he's not really into empirically testing those.
% Maybe you're thinking here more of the recent literatures (Kraft, Shipley et al) where people have tested for spacing out in trait space in one way or another?
...
% theory is not as relevant anyway. [MW: true that it doesn't connect to traits -- are we in fact going to come back to it at all?]
...
% unmeasurable traits or not easily measurable traits such as the R*
% and model based on easily measurable traits that were developed in
% the strategies (iii) (Grime Weiher and Westobty) of the intro is important here.
...
% % GEORGES: Not sure we need this section on adaptive-dynamics.
...
% structure the prediction are quite drastically different). I have
% try to move this bit here at it seem as the early step to Leimar.
% GEORGES: The section below do no seems crucial to me.
...
% GEORGES: This seems very similar to the section with the table with
% the list so I moved all the stuff on list of kernel used here.
...
%   measure of specialisation or generalisation as between two resources.
...
% MW: Generalist somehow seems the wrong word, implies either resource would be OK
%
...
% Not clear to me -- means the function is discontinuous?
...
strategies.
% MW: Vegetative growth certainly, is it really the population-level effect we're wanting to emphasize here?
...
the field.
%MW: I'd say rather, in the hope of making sensible predictions about composition of assemblages that can be tested
...
level is OK and equivalent to invasion fitness.
