...
%  
Two main classes of resource-based models have been investigated for plants. 
%
The first is Tilman's  \Rstar\ model \citep{Tilman-1977,Tilman-1982}, which focusses
on the minimal level at which individuals from a species can maintain growth. 
Similar to the kernel-based models, populations are not size-structured, so 
...
oriented. Strategy schemes arose as an attempt to classify the wide range 
...
\citep[e.g.][]{Wright-2010}.
% anything where kernels measured?
%
From these we can infer processes that may be structuring the community.
% 
The quality of these inferences thus depends crucially on the validity of
the insights drawn from the two other more-theoretical approaches.


Ideally, these different approaches to studying community assembly would
mutually reinforce each other, yet to date literatures remain unhelpfully
isolated from one another. 

% Material drawn straight from old work 
...
\end{itemize}
%Extends work of Abrams, who asked how R* connected with resource utilization curves. 
