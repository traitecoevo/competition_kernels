...
decreases with per-capita mortality rate $m$. Species-$i$ depletes resource-$j$
at rate $c_{ji} \,  N_i \, \mu_i(R_1,R_2)$, where $j = 1, 2$ and $c_{ji}$ is
the content of resource-$j$ in species-$i$. Resources
are replenished
with supply rate $S_j$ and decay at rate $D_j$. Under these assumptions, the
...
\end{align}
\end{subequations}
where $n$ is the number of species and $j=1,2$ is the number of resources.
